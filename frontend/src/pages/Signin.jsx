...
const Signin = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setStatus }) => {
      try {
        const response = await axiosInstance.post('/signin', values);  // No need to repeat the base URL
        setStatus({ success: 'Signin successful!' });
          ...
          <div className="w-full flex-1 mt-14">
            <form onSubmit={formik.handleSubmit} className="mx-auto max-w-xs">
              ...
              <button
                ...
                className="mt-8 tracking-wide font-semibold bg-[linear-gradient(to_right,_#6848c6,_#d84189,_#fb9438)] text-gray-100 w-full py-4 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none"
              >
                {formik.isSubmitting ? (
                  <>
